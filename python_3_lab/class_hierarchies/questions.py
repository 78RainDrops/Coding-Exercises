...
class TrueFalseQuestion(Question):
    ...
    def __prefix_if_necessary(self, question_text) -> str:
        if question_text.lower().startswith(
            "false"
        ) or question_text.lower().startswith("true"):
...
class MultipleQuestion(Question):
    def __init__(self, question_text, choices, correct_choice) -> None:
        question_text = self.__add_suffix(question_text, correct_choice)
        super().__init__(question_text, [True, False], correct_choice)

    def __add_suffix(self, question_text, correct_choice):
        return f"{question_text} (select {len(correct_choice)})"
