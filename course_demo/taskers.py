class Todo:
    ...
    new Todo should have a `completed` field that defaults to `False`.
    ...
    def __repr__(self):
        return "{} ({} - {} points): {}".format(
            self.name, self.statuses[self.completed], self.points, self.description
        )
...
class TodoList:
    ...
    >>> todo2 = Todo(name='Submit Talk Proposal', description='Write and submit talk for PyC    on', points=3)
    ...
    def average_points(self):
        ...
        for todo in self.todos:
            total += todo.points
        return float(total) / len(self.todos)
